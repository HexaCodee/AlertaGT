    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        logger.LogInformation("Token generado para verificación de email: {Token}, Longitud: {Length}", emailVerificationToken, emailVerificationToken.Length);
        ...
        _ = Task.Run(async () => // Propio de .Net para tareas en background, Fire-and-forget.
        {
            try
            {
    ...
    {
        // Normalizar el token: trimmed y validación
        var token = verifyEmailDto.Token?.Trim();
        
        logger.LogInformation("Token recibido para verificación: {Token}", token);
        logger.LogInformation("Longitud del token: {Length}", token?.Length ?? 0);
        
        if (string.IsNullOrWhiteSpace(token))
        {
            logger.LogWarning("Token vacío o nulo recibido");
        ...
        {
            logger.LogWarning("No se encontró usuario con el token normalizado: {Token}", token);
            return new EmailResponseDto
            {
                Success = false,
                Message = "Token de verificación inválido o expirado"
